...
data class GithubUser(
    ...
    val mt5Registrado: Boolean = true,
    val mt5IdConta: String = "",
    val licencaAtiva: Boolean = true,
    val licencaProduto: String = "Fimaster EA Pro",
    val licencaPlano: String = "vitalicia",
    val licencaValidade: String = "",
    val licencaUltimaRenovacao: String = "",
    val licencaTotalRenovacoes: Int = 0,
    val licencaHistorico: List<GithubUserHistorico> = emptyList(),
    val reembolsoSolicitado: Boolean = false,
    val reembolsoStatus: String = "NENHUM",
    val autorizacaoStatus: String = "APROVADO",
    val autorizacaoAprovadoPor: String = "SISTEMA",
...
fun EaRobotEvent.toEntity(): EaRobotEventEntity {
    val resolvedGid = if (gid.isNotBlank()) gid else EaEventGidManager.getOrCreateGid(this)
    return EaRobotEventEntity(
        gid = resolvedGid,
        id = id,
        currency = currency,
        event = event,
...
fun EaRobotEventEntity.toDomain(): EaRobotEvent {
    return EaRobotEvent(
        id = id,
        gid = gid,
        currency = currency,
...
fun EaRobotEvent.getResolvedGid(): String {
    if (gid.isNotBlank()) return gid
    return EaEventGidManager.getOrCreateGid(this)
}
